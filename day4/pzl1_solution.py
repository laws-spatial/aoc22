...
with open(file_path, "r") as f:
    ...
    while i < len(range_text_first):
        ...
        # index 0 is the same
        if (len(left) == len(right) and left[0] == right[0]):
                count += 1
        ...
        elif (len(left) > len(right) and left[0] <= right[0] and left[-1] >= right[-1]):
                    count += 1
    ...
    print(f"{count} assignment numbers completely contain their counterpart assignment.")

# What I learned:
# I learnd that you can chain list comprehension, which is
# not as readable but definitely super helpful. Additionally, I 
# chained conditional if statements for the first time.